...
# --- Configuration ---
$InputFile = "C:\temp\new-6th-grade-google.csv" 
$googleWorkspaceDomain = "collegebound.elhaynes.org"         
...
# Check if GAM executable exists
if (-not (Test-Path $gamPath)) {
    Write-Error "GAM executable not found at '$gamPath'. Please install GAM and update the `$gamPath` variable."
    exit
}
...
foreach ($user in $usersToCreate) {
    ...
    # Construct Google Workspace primary email address (e.g., firstname.lastname@yourdomain.com)
    # You might adjust this format based on your organization's naming convention.
