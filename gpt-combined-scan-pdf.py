...
def check_address_hash_in_pdf(pdf_path):
    try:
        ...
        # Define an area to the right of the label (same line)
        # Adjust width as needed (400 = ~4 inches at 100 dpi)
        addr_rect = fitz.Rect(label.x1, label.y0 - 5, label.x1 + 400, label.y1 + 5)

        # Extract text in that zone
...
# === Main logic to analyze all PDFs ===
